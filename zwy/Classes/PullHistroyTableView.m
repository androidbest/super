...
- (id)initWithFrame:(CGRect)frame WithDelegate:(id)delegate{
    ...
    if (self) {
        ...
        self.centerActivity =[[UIActivityIndicatorView alloc]
                              initWithFrame : CGRectMake(ScreenWidth/2.0f-32.0f/2.0f, -25.0f, 32.0f, 32.0f)];
        ...
        labelText.frame =CGRectMake(ScreenWidth/2.0f-150.0f/2, -25.0, 150, 20);
...
- (void)reloadData:(BOOL)animation{
    ...
    if (isInitTableView) {//保证该方法只调用一次
        ...
        if (self.contentSize.height>self.frame.size.height) {
            self.contentInset=UIEdgeInsetsMake(ContentInsetTop+InsetTop, 0, 0, 0);
        }else{
            self.contentInset=UIEdgeInsetsMake(InsetTop, 0, 0, 0);
            _reachedTheEnd=YES;
        }
    }
    
    //保证刷新后内容位置不变(通过插入前与插入后进行对比)
    if (updateContentHeightAfter-updateContentHeightBefore==0.0)self.contentOffset=CGPointMake(self.contentOffset.x,-InsetTop);
    else self.contentOffset=CGPointMake(self.contentOffset.x,updateContentHeightAfter-updateContentHeightBefore-ContentInsetTop+InsetTop);
