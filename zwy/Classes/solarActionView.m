...
- (void)initWithDate:(NSDate *)date{
    ...
    NSString *strMonth_date;
    strMonth_date =[NSString stringWithFormat:@"%dx%d",lunar.month,lunar.reserved];
    if (lunar.month<10)strMonth_date =[NSString stringWithFormat:@"0%dx%d",lunar.month,lunar.reserved];
    NSString *strDay_date =[NSString stringWithFormat:@"%d",lunar.day];
    if (lunar.day<10)strDay_date =[NSString stringWithFormat:@"0%d",lunar.day];
...
-(NSString*) pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    switch (component) {
        case 0:
            ...
            break;
        case 1:
            return [NSString stringWithFormat:@"%@%@",arrMonthList[row],@"月"];
            break;
        case 2:{
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex==0) {
        ...
        /*月*/
        NSString * month;
        if (indexMonth>=arrMonthList.count)month =[Date_string setMonthInt:arrMonthList[arrMonthList.count-1]];
        else month =[Date_string setMonthInt:arrMonthList[indexMonth]];
        
        /*日*/
        NSString * day;
        if (isBigMonth) {
            if (indexDay>=arrDayBigList.count) day =[Date_string setDayInt:arrDayBigList[arrDayBigList.count-1]];
            else day =[Date_string setDayInt:arrDayBigList[indexDay]];
        }else {
            if (indexDay>=arrDayLittleList.count) day =[Date_string setDayInt:arrDayLittleList[arrDayLittleList.count-1]];
            else day =[Date_string setDayInt:arrDayLittleList[indexDay]];
        }
       
        /*润*/
        NSString * reserved;
        if (indexMonth>=arrMonthList.count)reserved =[Date_string setReservedInt:arrMonthList[arrMonthList.count-1]];
        else  reserved =[Date_string setReservedInt:arrMonthList[indexMonth]];
        [_DetaSource actionSheetSolarDate:year Month:month Day:day reserved:reserved];/*返回小写的农历*/
        
        
        year =arrYear[indexYear];
        if (indexMonth>=arrMonthList.count)month=arrMonthList[arrMonthList.count-1];
        else  month =arrMonthList[indexMonth];
        
        if (isBigMonth) {
            if (indexDay>=arrDayBigList.count)day =arrDayBigList[arrDayBigList.count-1];
            else day =arrDayBigList[indexDay];
        }else{
            if (indexDay>=arrDayLittleList.count)day =arrDayLittleList[arrDayLittleList.count-1];
            else day =arrDayLittleList[indexDay];
        }
      
        
        [_DetaSource actionSheetTitleDateText:year Month:month day:day];/*返回大写的农历*/
    }
}
